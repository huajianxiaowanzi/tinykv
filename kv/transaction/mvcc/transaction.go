...
func (txn *MvccTxn) GetValue(key []byte) ([]byte, error) {
	...
	defer iter.Close()
	iter.Seek(EncodeKey(key, txn.StartTS)) // 第一个匹配的就是最新可见版本，无需全量扫描。
	if !iter.Valid() {
		return nil, nil
	}
...
// CurrentWrite 用于判断事务是否已有最终状态，防止对已完成的事务做重复或冲突操作。
//
//	 具体例子：
//	Commit(key, startTs=80) 流程：
//	  1. CurrentWrite(key) → 查找 startTs=80 的 Write
//	  2. 如果找到 Write.Kind=Put → 已提交 → 拒绝重复提交
//	  3. 如果找到 Write.Kind=Rollback → 已回滚 → 拒绝提交已回滚的事务
//	  4. 如果没找到 → 正常，继续提交流程
...
// 判断目标 Write 的 key 是不是我们需要的，不是返回空，是直接返回该 Write；
