...
❯ c  tinykv/
    ...
    ├── kv/                      # KV 存储核心                                                                              
    │   ├── server/              # gRPC 服务处理（Raw API + Transaction API）                                               
                                                                                                                            
    │   ├── storage/             # 存储引擎抽象（Standalone/Raft Storage）                                                  
    │   ├── raftstore/           # Multi-Raft 协调层（Peer、Region、Worker）                                                
    │   ├── transaction/         # MVCC 事务层                                                                              
    │   │   ├── mvcc/            # MVCC 核心实现（Lock/Write/Default CF）                                                   
    │   │   └── latches/         # 事务锁存器                                                                               
    │   └── util/engine_util/    # Badger CF 模拟（前缀编码）                                                               
    │                                                                                                                       
    ├── raft/                    # Raft 共识算法实现                                                                        
    │   ├── raft.go              # Raft 状态机（选举、日志复制）                                                            
    │   ├── log.go               # RaftLog 管理                                                                             
    │   ├── rawnode.go           # RawNode 接口                                                                             
    │   └── storage.go           # Raft 持久化接口                                                                          
    │                                                                                                                       
    ├── scheduler/               # 中央调度器（类似 TiDB PD）                                                               
    │   ├── server/              # Scheduler 服务                                                                           
    │   │   ├── cluster.go       # 集群元数据、心跳处理                                                                     
    │   │   └── schedulers/      # 均衡调度器                                                                               
    │   └── client/              # Scheduler 客户端                                                                         
    │                                                                                                                       
    ...
    │                                                                                                                       
    ├── log/                     # 日志工具                                                                                 
    ├── message/                 # 消息封装（Callback）                                                                     
    └── pkg/                     # 通用工具包 你将的这些互相之间是怎么调度的                                                

● 模块调用关系图：

┌─────────────────────────────────────────────────────────────┐                                                           
│                      Client (TinySQL)                        │
│                   发送 gRPC 请求（Raw/Transaction API）        │                                                        
└─────────────────────────────┬───────────────────────────────┘
│ gRPC
▼
┌─────────────────────────────────────────────────────────────┐
│                     kv/server                                │
│  RawGet/Put/Delete/Scan  ←→  KvGet/Prewrite/Commit/Rollback  │
│          (Raw API)                 (Transaction API)          │
└─────────────────────────────┬───────────────────────────────┘
│
┌───────────────────┼───────────────────┐
│                   │                   │
▼                   ▼                   ▼
┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐
│ StandaloneStorage│  │   RaftStorage   │  │   MvccTxn       │
│   (单机存储)     │  │  (分布式存储)    │  │  (事务层)       │
└────────┬────────┘  └────────┬────────┘  └────────┬────────┘
│                    │                    │
│                    ▼                    │
...
│                               │
▼                               ▼
Client 获取时间戳              kv/raftstore 心跳上报
Region 路由查询               Region 元数据更新
...
│    API 类型     │                                调用链                                 │
├─────────────────┼───────────────────────────────────────────────────────────────────────┤
│ Raw API         │ Client → kv/server → StandaloneStorage → Badger                       │
├─────────────────┼───────────────────────────────────────────────────────────────────────┤
│ Transaction API │ Client → kv/server → MvccTxn → StandaloneStorage/RaftStorage → Badger │
├─────────────────┼───────────────────────────────────────────────────────────────────────┤
│ Raft API        │ Client → kv/server → RaftStorage → raftstore → raft → Apply → Badger  │
└─────────────────┴───────────────────────────────────────────────────────────────────────┘
...
│          交互           │                          说明                           │
├─────────────────────────┼─────────────────────────────────────────────────────────┤
│ server ↔ storage        │ server 调用 Storage.Write/Reader，存储层执行读写        │
├─────────────────────────┼─────────────────────────────────────────────────────────┤
│ server ↔ MvccTxn        │ Transaction API 请求由 MvccTxn 处理 MVCC 编码           │
├─────────────────────────┼─────────────────────────────────────────────────────────┤
│ RaftStorage ↔ raftstore │ RaftStorage 将请求发送到 raftWorker，走 Raft 共识       │
├─────────────────────────┼─────────────────────────────────────────────────────────┤
│ raftstore ↔ raft        │ raftWorker 调用 RawNode.Propose/Ready，驱动 Raft 状态机 │
├─────────────────────────┼─────────────────────────────────────────────────────────┤
│ raftstore ↔ scheduler   │ 心跳上报 Region 状态，接收调度指令（Balance/Split）     │
├─────────────────────────┼─────────────────────────────────────────────────────────┤
│ scheduler ↔ Client      │ Client 从 scheduler 获取时间戳和 Region 路由            │
└─────────────────────────┴─────────────────────────────────────────────────────────┘
